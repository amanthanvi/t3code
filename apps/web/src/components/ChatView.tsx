...
function ChatViewContent(props: ChatViewProps) {
  ...
  // Compaction gating follows the driver that will actually run the turn,
  // falling back to the requested kind before any entry resolves.
  const activeProviderDriverKind = activeProviderEntry?.driverKind ?? requestedProvider;
  ...
  // `codex_personal`) is tracked rather than the default Codex's. Falls back
  // to first-match-by-kind when no saved instance id is available or the
  ...
  const compactDisabled =
    ...
    !activeProject ||
    !isServerThread ||
    activeProviderDriverKind !== "claudeAgent" ||
  ...
  const resumeCompactionBannerItem = useMemo<ComposerBannerStackItem | null>(() => {
    if (
      ...
      phase === "running" ||
      !shouldOfferResumeCompaction({
        provider: activeProviderDriverKind,
